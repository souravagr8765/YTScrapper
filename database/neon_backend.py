"""
Neon PostgreSQL backend (primary remote database).

Uses psycopg2 with a connection pool. All operations are synchronous but
wrapped in try/except so a Neon outage never crashes the app — the caller
receives None / empty list and the reconciler will backfill on the next run.
"""
import logging
...
class NeonBackend:
    def __init__(self, dsn: str):
        if not dsn:
            raise ValueError("NEON_DATABASE_URL is not set — Neon backend disabled.")
        # Small thread-safe pool; min=1 max=3 keeps Neon free-tier happy
    ...
    def upsert_video(self, video: Video) -> bool:
        """Returns True on success, False if Neon is unreachable."""
        try:
    ...
    def get_videos_since(self, since: datetime) -> List[Video]:
        """Delta fetch — only records newer than `since`. Avoids full-table scans."""
        try:
            with self._get_conn() as conn:
            ...
            logger.warning("Neon get_videos_since failed: %s", exc)
            return []
    ...
    def get_all_videos(self) -> List[Video]:
        try:
            ...
            logger.warning("Neon get_all_videos failed: %s", exc)
            return []
    ...
    def get_max_updated_at(self) -> Optional[datetime]:
        try:
            ...
            logger.warning("Neon get_max_updated_at failed: %s", exc)
            return None
